    ...
    <article class="content-card" style="backdrop-filter: blur(80px) saturate(75%)">
      ...
      <div class="mb-12 flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
        ...
        <div class="flex flex-col items-start gap-4 md:items-end">
          <div class="flex flex-col gap-1.5 text-sm text-slate-400 md:text-right">
            <a :href="resumeData.contact.emailHref" class="transition hover:text-cyan-300">{{
              resumeData.contact.email
            }}</a>
            <a
              :href="resumeData.contact.githubHref"
              target="_blank"
              rel="noreferrer"
              class="transition hover:text-cyan-300"
              >{{ resumeData.contact.github }}</a
            >
            <a
              :href="resumeData.contact.linkedinHref"
              target="_blank"
              rel="noreferrer"
              class="transition hover:text-cyan-300"
              >{{ resumeData.contact.linkedin }}</a
            >
          </div>
      ...
      <section class="resume-section">
        ...
        <div class="resume-card">
          <ul class="grid gap-2">
            <li
              v-for="item in resumeData.summary"
              :key="item"
      ...
      <section class="resume-section">
        ...
        <div class="resume-card">
          <ul class="grid gap-2">
            <li
              v-for="tech in resumeData.technologies"
              :key="tech.category"
        ...
        <div class="resume-card">
          <div class="grid grid-cols-3 gap-4">
            <div v-for="lang in resumeData.languages" :key="lang.language" class="text-center">
              <p
                class="text-xs font-semibold uppercase tracking-[0.22em] text-cyan-300/60"
